<template>
  <DefaultLayout title="Consistency">
    <FilterBar>
      <DropdownSelect
    ...
    <PageSection class="consistency__summary">
      <div class="consistency__gauge-block">
        <GaugeChart :value="totalConsistent" :max="totalCount" />
        <div class="consistency__gauge-caption">
          <p class="consistency__gauge-caption-label">Total</p>
          <p class="consistency__gauge-caption-value">
            {{ totalConsistent }}
            <span class="consistency__gauge-caption-value-muted">/{{ totalCount }}</span>
          </p>
        </div>
      </div>
    ...
    <PageSection no-padding>
      ...
      >
        <template #toolbar-left>
          <div class="consistency__toggle-field">
            <ToggleSwitch v-model="showAllResults" label="Show all results" @update:modelValue="handleToggleShowAll" />
            <span>Show all results</span>
          </div>
          <InfoTooltipButton message="서로 연관된 컬럼 간 값이 일치하는지를 기준으로 정합성을 계산합니다." />
        </template>

        <template #export>
          <ExcelExportButton :data="filteredRows" file-name="consistency" />
        </template>

        <template #cell-consistency="{ row }">
          <span class="consistency__cell-consistency">
            <StatusDot :tone="rowOf(row).tone" />
            <span
              class="consistency__cell-consistency-text"
              :class="`consistency__cell-consistency-text--${rowOf(row).tone}`"
            >
              {{ rowOf(row).consistencyNum }}/{{ rowOf(row).consistencyDen }}
            </span>
          </span>
        </template>

        <template #cell-errorColumn="{ value }">
          <span class="consistency__cell-errors">
            <template v-if="errorsOf(value).length">
              <Badge v-for="(err, index) in errorsOf(value)" :key="index" :tone="err.tone">
                {{ err.label }}
              </Badge>
            </template>
            <span v-else class="consistency__cell-errors-empty">-</span>
          </span>
        </template>
      </DataTable>
    </PageSection>
...
// C/C ATA 기준 데이터 정합성(연관 컬럼 간 값이 서로 일치하는지)을 보여주는 화면입니다.
// Completeness 화면과 레이아웃/컴포넌트 구성이 동일해서(게이지 + Sea/Rail/Air 요약 카드 +
// 필터 가능한 표), ModeSummaryCard(+Group) 등 공용 컴포넌트를 그대로 재사용합니다.
...
function handleSearch(): void {
  ...
  console.log('search consistency', { ...filters })
}
...
interface ConsistencyRow {
  ...
  mblNo: string
  mode: TransportMode
  consistencyNum: number
  consistencyDen: number
  tone: StatusDotTone
...
function toneOf(num: number): StatusDotTone {
  if (num >= CONSISTENCY_DEN) return 'good'
  if (num >= 12) return 'warning'
  return 'danger'
}

function buildRow(index: number): ConsistencyRow {
  // Completeness 화면과 겹치지 않는 시드 오프셋을 써서, 같은 index라도 다른 값이 나오게 합니다.
  const seed = index * 13.7 + 4001
  const num = Math.max(8, CONSISTENCY_DEN - Math.floor(seededRandom(seed) * 7))
  ...
  return {
    ...
    mode,
    consistencyNum: num,
    consistencyDen: CONSISTENCY_DEN,
    tone,
    errorColumn,
  }
}

const consistencyRows = Array.from({ length: ROW_COUNT }, (_, index) =>
  buildRow(index),
) as unknown as Record<string, unknown>[]

const filteredRows = computed(() =>
  consistencyRows.filter((row) => (row as unknown as ConsistencyRow).mode === selectedMode.value),
)

function rowOf(row: Record<string, unknown>): ConsistencyRow {
  return row as unknown as ConsistencyRow
}
...
function rowToneClass(row: Record<string, unknown>): string | undefined {
  ...
  if (tone === 'warning') return 'consistency__row--warning'
  if (tone === 'danger') return 'consistency__row--danger'
  return undefined
}
...
.consistency__gauge-caption-label {
  margin: 0 0 4px;
  font-size: $font-size-sm;
  color: $color-text-muted;
}

.consistency__gauge-caption-value {
  margin: 0;
...
.consistency__gauge-caption-value-muted {
  font-size: $font-size-base;
  font-weight: $font-weight-bold;
  color: $color-text-muted;
}

.consistency__toggle-field {
  display: flex;
  align-items: center;
  gap: $spacing-xs;
  font-size: $font-size-sm;
...
.consistency__cell-consistency {
  display: inline-flex;
  align-items: center;
  gap: 6px;
}

.consistency__cell-consistency-text {
  font-weight: $font-weight-bold;
...
.consistency__cell-errors-empty {
  color: $color-text-muted;
}

:deep(.consistency__row--warning) .data-table__td {
  background: rgba($color-warning, 0.08);
}

:deep(.consistency__row--danger) .data-table__td {
